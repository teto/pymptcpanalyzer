...
# def _peers_fields(fields):
#     return _sender(fields) + _receiver(fields)
...
# TODO generate this from the fields
# flag depending on NAT detection/
hashing_fields = [
    # We shouldn't have to hash on these (in case of NAT etc)
    "ipsrc",
    "ipdst",
    "sport",
    "dport",
...
artificial_fields = {
    ...
    "mptcpdest" : Field("mptcpdest", dtype_role, "MPTCP destination", False),
    "tcpdest" : Field("tcpdest", dtype_role, "TCP destination", False),
    "hash": Field("hash", np.float64, "Hash of fields", False),
}
...
def load_into_pandas(
    ...
    **extra
    ...
    tshark_dtypes = {fullname: field for fullname, field in config.fields().items()
...
def merge_tcp_dataframes_known_streams(
    ...
    # total = total.reindex(columns=firstcols + list(filter(lambda x: x not in firstcols, total.columns.tolist())))
    log.info("Resulting merged tcp dataframe of size {} ({} mapped packets vs {} unmapped)  with input dataframes of size {} and {}.".format(
        len(total),
    len(total[total._merged == "both"]), len(total[total._merged != "both"]),
        len(h1_df), len(h2_df)
    ))

    # print unmapped packets
    log.debug(total[total._merged != "both", _sender(TCP_DEBUG_FIELDS) + _receiver(TCP_DEBUG_FIELDS) ])
